...
def test_file_recording_links_rejected_and_dataset_links_retained(tmp_path):
    ...
    for kind in sorted(FILE_RESOURCE_KINDS):
        resource = db.create_data_resource(category="file", provider='local', namespace='files', source_key=kind, kind=kind, metadata={'license': 'upstream'})
        for key in ('session_id', 'recording_session_id'):
            with pytest.raises(ValueError, match='Files cannot be linked'):
                db.create_data_resource(category="file", provider='local', namespace='files', source_key=kind + key, kind=kind, metadata={key: 'recording'})
            with pytest.raises(ValueError, match='Files cannot be linked'):
    ...
    dataset = db.create_data_resource(category="dataset", provider='collection', namespace='datasets', source_key='demo', kind='demonstrations', metadata={'session_id': 'original'})
...
def test_migration_removes_only_file_ownership_keys(tmp_path):
    ...
    dataset = db.create_data_resource(category="dataset", provider='collection', namespace='datasets', source_key='demo', kind='demonstrations', metadata=metadata)
    file = db.create_data_resource(category="file", provider='local', namespace='files', source_key='assets', kind='simulation_assets')
...
def test_dataset_sources_are_distinct_and_exposed_in_list_and_detail(tmp_path):
    db = Database(tmp_path / 'registry')
    resource = db.create_data_resource(category="dataset", provider='collection', namespace='datasets', source_key='combined', kind='demonstrations', metadata={'session_id': 'a'})
