...
async function loadLocalCollection(force = false) {
  ...
  try {
    ...
    const reviewCount = data.captures.filter(capture => capture.summary.warnings.length).length;
    document.querySelector('#local-capture-count').textContent = `${data.captures.length} recording${data.captures.length === 1 ? '' : 's'}${reviewCount ? ` · ${reviewCount} to review` : ''}`;
    document.querySelector('#local-capture-body').innerHTML = data.captures.length ? data.captures.map(capture => {
      ...
      return `<tr><td>${escapeHtml(summary.header.task)}<span class="secondary">${escapeHtml(formatDate(summary.header.created_at))}</span></td>
        ...
        <td><a href="/api/collection/local/captures/${encodeURIComponent(capture.sha256)}/download" download>Export original</a></td></tr>`;
    }).join('') : '<tr><td colspan="4">No recordings imported. Record on the headset, share the file to this Mac, then import it above.</td></tr>';
  } catch (error) {
    document.querySelector('#local-capture-count').textContent = 'Could not refresh';
