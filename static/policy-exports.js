...
(() => {
  ...
  async function refreshNow() {
    ...
    try {
      ...
      if (dialog.open) error("policy-export-error", e.message);
      else if (detail.open) error("prepared-dataset-error", e.message);
      else {
  ...
  function preparedRow(job) {
    ...
    const state = failed ? "FAILED" : job.training_ready ? "READY" : job.state === "READY" ? "PREPARED" : "RUNNING";
    const actions = [];
    if (job.version_id && local) {
    ...
    if (job.state === "FAILED") actions.push(`<button type="button" class="text-button" data-preparation-retry="${esc(job.id)}">Retry</button>`);
  ...
  async function action(event) {
    ...
    try {
      ...
      if (data.preparationDelete || data.preparationDeleteFormat) {
        ...
        const target = wholeDataset ? "this dataset’s prepared versions" : "this prepared version";
        ...
        const path = wholeDataset ? `/datasets/${encodeURIComponent(data.preparationDelete)}` : `/${encodeURIComponent(data.preparationDeleteFormat)}`;
