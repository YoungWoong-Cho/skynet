...
(() => {
  ...
  function setBusy(value) {
    ...
    dialog.querySelector("[data-dialog-close]").disabled =
      dialog.dataset.blockClose === "true";
    confirm.disabled = value || !current || current.blockers?.length > 0;
    content.querySelectorAll("input").forEach((input) => {
      input.disabled = value || input.dataset.protected === "true";
    });
  }
  ...
  async function open(kind, id) {
    ...
    current = null;
    title.textContent =
      kind === "storage" ? "Inspect cluster files" : `Delete ${labels[kind]}`;
    ...
    try {
      ...
      if (kind === "storage") {
        ...
        if (!result.items.length)
          message.textContent =
            "No unreferenced files found in the inspected output directories.";
        for (const item of result.pending_deletions || []) {
          const button = document.createElement("button");
          button.type = "button";
          button.className = "button button-outline";
          button.dataset.deleteKind = item.kind;
        ...
        message.textContent =
          "Delete these dependencies first, or finish the active work:";
        content.innerHTML = table(
          ["Item", "Required action"],
          result.blockers.map(
            (item) =>
              `<tr><td>${item.id ? valueHtml(linkedValue(item.kind, item.id, item.label)) : escapeHtml(item.label)}</td><td>${escapeHtml(item.reason)} ${item.id && labels[item.kind] ? `<button type="button" class="button button-outline" data-delete-kind="${item.kind}" data-delete-id="${escapeHtml(item.id)}">Review deletion</button>` : ""}</td></tr>`,
          ),
        );
      } else {
        message.textContent =
        ...
        const counts = Object.entries(result.counts)
          .map(
            ([name, count]) =>
              `${count} ${{ adapters: "adapter versions", evaluation_suites: "suite versions" }[name] || name.replaceAll("_", " ")}`,
          )
          .join(" · ");
        const paragraph = document.createElement("p");
        paragraph.textContent = counts;
        ...
        content.insertAdjacentHTML(
          ...
          table(
            ...
            result.files.map(
              (item) =>
                `<tr><td>${escapeHtml(item.path)}${item.exists === false ? " (already absent)" : ""}</td><td>${bytes(item.size_bytes)}</td></tr>`,
            ),
          ),
        );
        confirm.textContent = result.retry
          ? "Retry deletion"
          : "Delete permanently";
      }
  ...
  function updateSelection() {
    if (current?.kind === "storage")
      confirm.disabled =
        busy || !content.querySelector("input:checked:not(:disabled)");
  }
  content.addEventListener("change", updateSelection);
  dialog.addEventListener("cancel", (event) => {
    if (dialog.dataset.blockClose === "true") event.preventDefault();
  });
  ...
  document
    .querySelector("#inspect-cluster-storage")
    .addEventListener("click", () => open("storage"));
  confirm.addEventListener("click", async () => {
    if (busy || !current || current.blockers?.length) return;
    const plan = current;
    const paths = [...content.querySelectorAll("input:checked")].map(
    ...
    try {
      await api(
        plan.kind === "storage"
          ? "/api/maintenance/storage/cleanup"
          : `/api/maintenance/history/${plan.kind}/${encodeURIComponent(plan.id)}`,
        {
