...
(() => {
  ...
  async function sessionRequest(method = "GET", payload) {
    const response = await originalFetch("/api/workspace/session", {
      ...
      headers: { "Content-Type": "application/json", ...(workspace ? { "X-Skynet-Workspace": workspace.id } : {}) },
      ...(payload ? { body: JSON.stringify(payload) } : {}),
    });
    const result = await response.json();
    if (!response.ok) throw new Error(typeof result.detail === "string" ? result.detail : "Unable to open workspace. Try again.");
  ...
  function installWorkspaceFetch() {
    window.fetch = async (input, options = {}) => {
      ...
      const signal = options.signal || (typeof input !== "string" ? input.signal : null);
      ...
      try {
        ...
        if (response.status === 401 || response.status === 409) {
          ...
          if (["workspace_required", "workspace_changed"].includes(result.code)) {
            leave(result.code === "workspace_changed" ? "Workspace changed in another tab." : "Enter your email to continue.");
  ...
  async function loadApplication() {
    ...
    for (const placeholder of document.querySelectorAll("script[data-workspace-src]")) {
      await new Promise((resolve, reject) => {
        ...
        script.onerror = () => reject(new Error("The app could not load. Reload this page to retry."));
  ...
  document.getElementById("workspace-sign-out").addEventListener("click", async event => {
  ...
  async function start() {
    ...
    try {
      ...
      message.textContent = new URL(location.href).searchParams.has("workspace_notice")
