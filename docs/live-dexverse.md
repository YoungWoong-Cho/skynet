...
5. When the scene opens, hold your right hand in front of you. Play calibrates and records, Stop pauses, and Reset discards the active attempt. The client sends these as START, STOP and RESET commands, respectively. A demonstration saves after the task success condition holds for ten steps.

For this task, lift the stick at least 20 cm above its default starting height and hold it within 30 degrees of upright. Keep recording until it saves and the session closes. Stop only pauses: it does not save the active attempt. Reset discards that attempt. Completed demonstrations remain on disk when a session stops.
