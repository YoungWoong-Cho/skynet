-- Recording deletion uses the same durable intent as other history.
...
FOR plan IN SELECT plan_json::jsonb FROM maintenance_operations LOOP
    ...
    FOR field IN SELECT * FROM jsonb_each_text(fields) LOOP
        ...
        IF plan->>'kind' IN ('adapter','suite','recording') AND right(field.key,5)='_json' THEN
...
BEGIN
FOREACH name IN ARRAY ARRAY['live_xr_sessions','live_conversions','policy_exports','data_resources','data_resource_versions','data_bundles'] LOOP
